...
[HoverPocket Windows 0.2.3 Setup.exeをダウンロード](https://github.com/shotaro311/hover-pocket/releases/download/win-v0.2.3/HoverPocketWin-win-Setup.exe)

インストールせずに試す場合は、[Portable ZIP](https://github.com/shotaro311/hover-pocket/releases/download/win-v0.2.3/HoverPocketWin-win-Portable.zip)を利用できます。Windows版0.2.3はWindows 11 x64向けの公開ベータです。現時点ではAuthenticode未署名のため、初回起動時にMicrosoft Defender SmartScreenの警告が表示される場合があります。1.0正式版ではタイムスタンプ付きAuthenticode署名を必須にします。
...
- macOS版ではCalendarの下段全幅を天気エリアとして区切り、左側に選択地域の本日の天気、右側に見やすい今後7日間の予報を表示します。
- Calendarパネルを開くと、本日の天気アイコンから週間予報へ順に一度だけ表示されます。本日のアイコンは、macOS 15以上では晴れのRotate、晴れ時々曇りの太陽だけのRotate、曇り・霧のBreathe、雨のVariable Color、雪のWiggle、雷のPulseを約5秒だけ再生します。macOS 14ではPulse / Variable Colorへ自動的に戻ります。週間予報はアイコンの形にかかわらず曜日・アイコン・数値の各行が揃います。繰り返し再生せず、macOSの「視差効果を減らす」が有効な場合はアニメーションなしで即時表示します。
- 上段右側の予定詳細と編集フォームは、内容が下段の天気エリアまで達する場合にスクロールできます。
- 天気の地域はSettingsから、現在地、世界の都市・郵便番号検索、日本47都道府県の簡易選択で設定できます。選択地点と直近の予報をこのMacへ保存します。
- 通信できない場合は保存済み予報を警告付きで表示し、保存済み予報もない場合は再試行表示へ切り替えます。

### クリップボード

- 「すべて」と「お気に入り」の2タブで表示します。どちらのタブもテキストを左、画像を右へ等分した split view です。
- テキストと画像の各履歴カードは星アイコンでお気に入り登録できます。
- お気に入り登録した履歴は、通常の履歴クリアでは消えません。
- どちらのタブでも、各カードのゴミ箱アイコンから履歴項目を個別に削除できます。
- 履歴項目をクリックすると、パネルいっぱいに拡大表示します。再度クリックすると閉じます。
- 長いテキストの拡大表示はスクロールできます。
- コピーアイコンをクリックすると、再びクリップボードへコピーできます。
...
- パネルサイズ: `Small / Medium / Large / Extra Large`。`Small=520x372`、`Medium=600x430`、`Large=680x488`、`Extra Large=760x546` です。
- ホバーパネル内の文字サイズ: `Small / Medium / Large / Extra Large`
- 表示するプロバイダーの ON / OFF
- プロバイダーの表示順
...
- Calendar天気の表示地域（現在地、世界の都市・郵便番号、日本47都道府県）
- Calendar天気の温度単位（自動 / ℃ / ℉）
...
Clipboard のお気に入り保護と旧履歴 JSON 互換を確認する場合は次のコマンドを使います。

```bash
.build/debug/HoverPocket --verify-clipboard
```

Timerの既定値、時間計算、start / pause / resume / stop、pin / unpin、保存先隔離、4段階の横並び幅を確認する場合は次のコマンドを使います。

```bash
.build/debug/HoverPocket --verify-timer
```

天気APIへの実接続、世界都市検索、47都道府県からの旧設定移行、地域設定の保存、オフラインキャッシュ、地点別タイムゾーン、温度単位、7日予報、SwiftUI描画を確認する場合は次のコマンドを使います。
...
- 初期値は東京都（`13`）です。既存の都道府県コード設定は、更新後も同じ代表地点へ自動移行します。
- Settingsでは「現在地を使用」、Open-Meteo Geocoding APIによる世界の都市・郵便番号検索、日本47都道府県の簡易選択を利用できます。検索地点はGeoNames由来の地点ID、名前、国・行政区、座標、タイムゾーンとともに保存します。
- 現在地はボタンを押した時だけCore Locationの許可を求め、取得した座標をOpen-Meteoへ送ります。常時追跡やバックグラウンド監視は行いません。
- Open-Meteo Forecast APIから、地点ごとの`timezone=auto`で当日と今後7日間の計8日分を取得します。温度単位は自動 / ℃ / ℉から選べ、APIキーや秘密情報は使いません。
- 直近の予報は地点と温度単位ごとに保存します。都市を移動した場合や温度単位を変えた場合に、別地点・別単位のキャッシュを誤表示しません。
- 取得データはOpen-Meteoへの帰属表示付きです。無料APIは非商用利用向けのため、商用配布へ切り替える場合は有料APIまたはセルフホストへの移行確認が必要です。
- Windows版には今回の天気UIをまだ追加していません。Windows側へ展開する際は同じ保存モデル、世界都市検索、自動タイムゾーン、温度単位、表示仕様を基準にします。
