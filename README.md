...
インストールせずに試す場合は、[Portable ZIP](https://github.com/shotaro311/hover-pocket/releases/download/win-v0.2.3/HoverPocketWin-win-Portable.zip)を利用できます。Windows版0.2.3はWindows 11 x64向けの公開ベータ候補です。現時点ではAuthenticode未署名のため、初回起動時にMicrosoft Defender SmartScreenの警告が表示される場合があります。1.0正式版ではタイムスタンプ付きAuthenticode署名を必須にします。
