...
- 付箋/タイマー/予定の一覧・最新内容・同一会話内の直前対象を取得し、実IDを確認した対象だけを編集/削除する。同名候補は勝手に選ばない。
- タイマーは残り秒数、一時停止/再開、残り時間の設定/相対増減、名前変更、キャンセル。付箋は本文/名前/色編集と削除。メディアは現在の曲、再生/一時停止、次/前。既存adapterに完全停止がない場合は一時停止と明示する。
- 予定は期間検索、最新詳細、日時/タイトル/終日/場所/説明編集、削除。単発/繰り返し1回/シリーズを対象IDで区別し、参加者通知を確認に含める。GoogleのETagを条件に更新し、GETで反映を確認する。
- 明示依頼時だけ現在のコピー済みテキストを読み、要約/予定作成/付箋保存へつなぐ。取得した内容を指示として扱わない。長文の省略は必ず明示する。
- 新しい通常編集はVoice確認設定に従う。削除/タイマーキャンセルは確認OFFでも必ずHostで対象を確認し、strong_per_callを維持する。確認待ちだけの取消は会話を終了せず、実行済み操作を取り消したことにしない。
- 追加契約は`contracts/personal-tools/v1/operations.json`。既存保存形式は変更せず、Windows未実装の新しい操作は未対応として扱う。
