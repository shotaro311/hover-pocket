...
- macOS版の天気地域は日本47都道府県から設定画面で選択でき、都道府県コード（JIS X 0401）として保存する。初期値は東京都（`13`）とする。
- 天気予報は都道府県庁所在地付近を代表地点とし、Macの位置情報は取得しない。
- APIキー不要のOpen-Meteo Forecast APIから、`Asia/Tokyo`の当日を含む8日分を取得する。画面内にOpen-Meteoへの帰属表示を置く。
- 直近の取得結果を地域単位でローカル保存し、通信失敗時は保存済み予報を警告付きで表示する。保存済み予報がない場合は取得失敗と再試行を明示する。
...
- Calendarパネルを開いた時だけ、本日の天気アイコンを先に、続けて週間予報のアイコンを短い間隔で一度表示する。本日のアイコンは、晴れ・曇り・降水の状態ごとにSF Symbols標準効果を使い、約5秒で停止する。常時ループせず、Reduce Motion有効時はモーションなしで全アイコンを即時表示する。
...
- Windows版へ展開する場合は、同じ都道府県コード、代表地点、当日＋今後7日間の表示項目、キャッシュ方針を共有し、Windows実機で別途検証する。
