...
- helper executable pathはモデルtoolのfilesystem permissionで明示denyする。production有効化前に、非機密surrogateを使う実runtime canaryで、2回のResponses requestに対してhelper起動が1回だけであること、remote `/models`アクセスがないこと、Codex auth control-planeからhelperを起動できる一方でモデル要求toolから同じpathを読取り・実行できないこと、request body、auth stdout、process state、Codex Home、log、diskへcredentialが残らないことをreadbackする。
