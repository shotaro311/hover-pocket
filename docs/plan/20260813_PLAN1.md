...
**ステータス**: 実装中・AN5-C PR #18とAN3-A PR #19は両OS CI成功・人手merge待ち。AN3-B1 Windows実音声transportをstacked worktreeで実装中
...
**現在地**: AN3-B1 - AN3-A head `b34c1fc`を土台に、Windowsの明示microphone操作、WebRTC、Codex experimental Realtime、remote audio、transcript、mute / stopをdefault-offのまま接続中

**次のアクション**: AN3-B1をWindows Release / fake app-server / rendered WebView / security / 実機Codex schema・音声1往復で検証する。その後AN3-B2でCalendar + TimerをCapability Brokerへ接続し、macOS parityへ進む
...
- [ ] AN3: Voice LaneとCalendar + Timer縦断（AN3-A foundationはPR #19でCI済み。AN3-B1 Windows実音声を実装中、Broker Tool AdapterはAN3-B2）
