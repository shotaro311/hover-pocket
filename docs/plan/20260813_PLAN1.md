...
**ステータス**: 実装中・AN3-B2 Draft PR #22でWindows Calendar / Timer Broker sliceを実装し、安全監査で検出したtool隔離・Calendar grant・Timer承認queueを修正中。現行Codex 0.145.0は正のtool policy非対応のためproduction Voiceはfail closed

**前バージョンからの変更**: AN3をdefault-off foundationと実音声接続へ分割し、AN5-C最新head上でVoice Laneを先行実装する現在地へ更新した。
...
**現在地**: AN3-B2 - WindowsのCalendar read / Timer startをCapability Registry / Brokerへ接続済み。Calendarは別Host grant、Timerは毎回native承認とreadbackを使う。Codex tool surfaceの正のallowlistが未提供のためproduction activationは閉じたまま

**次のアクション**: AN3-B2のWindows Release / rendered WebView / security再監査を完了し、Draft PRでfail-closed境界を固定する。並行して、OpenAI公式の正のtool allowlistまたはBrokerだけを公開する専用最小runtimeを選定し、解除条件を満たした後に実音声E2EとmacOS parityへ進む
...
- [ ] AN3: Voice LaneとCalendar + Timer縦断（AN3-A / B1はCI済み。AN3-B2のBroker Tool AdapterはDraft実装済みだが、Codex 0.145.0に正のtool policyがないためproduction Voiceは安全停止中）
