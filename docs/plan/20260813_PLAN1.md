...
**現在地**: AN4 - Pocket App DSL、両OSの汎用Renderer、宣言型Today Focus packageの実装と検証が完了

**次のアクション**: AN4をmainへ統合し、AN5でCodex生成、検証、プレビュー、権限差分承認、immutable install、rollbackを実装する

**ブロッカー**: なし。AN5で生成packageを受け入れる前に、immutable install snapshotと安全なstableKey表示を必須gateとして実装する
