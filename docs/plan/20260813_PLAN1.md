...
**ステータス**: 実装中・AN3-B2 Draft PR #22のWindows Calendar / Timer Broker slice、安全監査、Provider識別子の最小化、Voice設定直列化はCI済み。現行Codex 0.145.0は正のtool policy非対応のためproduction Voiceはfail closed

**前バージョンからの変更**: AN3-B2のBroker限定gate、Calendar別権限、Timer native承認、Provider識別子非送信、Voice設定直列化の実装・CI・Security受け入れを反映した。
...
**現在地**: AN3-B2 - WindowsのCalendar read / Timer startをCapability Registry / Brokerへ接続し、Calendar別Host grant、Timer毎回native承認、readback、識別子最小化、設定直列化までCI / Security受け入れ済み。Codex tool surfaceの正のallowlistが未提供のためproduction activationは閉じたまま

**次のアクション**: Core Integration Gateの残差を実コードで再監査し、OpenAI公式の正のtool allowlistまたはBrokerだけを公開する専用最小runtimeを選定する。解除条件を満たした後に実音声E2EとmacOS parityへ進む
...
- [ ] AN3: Voice LaneとCalendar + Timer縦断（AN3-A / B1 / B2の実装・CI・Security受け入れは済み。Codex 0.145.0に正のtool policyがないためproduction Voiceと実音声E2Eは安全停止中）
- [x] AN4: Pocket App DSLと汎用Renderer
- [ ] AN5: Codexによる生成・検証・プレビュー・導入（AN5-A / B / CのHost pipeline、lifecycle、runtime activationは実装・CI済み。PR #18の統合、実Codex confinement、Voice E2Eが未完了）
...
ユーザーはSettingsの「AI・自動操作履歴」からreceiptを確認・削除・exportできるようにする。保持期間は`7日 / 30日 / 90日 / 無期限`、既定90日とする。履歴削除では監査ログとreceipt内容を消す一方、pending recordとidempotency衝突検査に必要なplan / argument / capability digestの実行済み墓標を残す。redact済みreceiptの再要求は`unknown`で停止し、副作用を再実行しない。audit fileのmalformed name、symlink / reparse point、破損台帳はfail closedにする。
...
Voice Laneは生成Custom UIではなくHost-owned trusted surfaceである。Windows AN3-B1では既存Panelのtrusted virtual host `https://app.hoverpocket.local`をexact originとして固定し、feature enabled、Panel可視、Hostが発行する1回限りのgesture lease、WebView2のuser-initiated Microphone requestの全条件を満たす場合だけpermission handlerが許可する。Settings、生成Custom UI、background、別origin、Cameraは拒否し、permissionをprofileへ保存しない。
...
- 右側に現在root配下の表示session数、mute、明示expand controlを置く。Voice sessionの開始・接続キャンセル・終了・再開は同じ大きなマイクcontrolで行う。
...
- muteは音声入出力だけを止める。hover closeはmute + UI detachとし、root / child taskをcancelしない。同じマイクcontrolによる終了操作はRealtime音声sessionだけを終了し、root / child taskを継続する。再ホバー後は明示的に同じマイクcontrolを押した場合だけ既存sessionを再開する。task cancelはsession card上の別操作として対象・影響を表示し、別承認を求める。
...
AN3の初期Windows実装はapp-serverのdynamic toolを`AgentToolAdapter`として使い、Registry descriptorをHost-owned toolへ変換する。MCPはAN7の外部Adapterに限定し、Voice内部正本にしない。

- Registry descriptorからHostが許可したdynamic tool metadataだけを生成する。
- Calendar readはGoogle接続やMicrophoneと別のpersisted / revocable Host grantを必要とし、許可前はtool定義にも含めない。
- Timer writeはexact argumentsのnative per-call approval、single-flight、1分3件のpre-presentation rate limit、verified readbackを必要とする。
- tool callはroot / call correlationとsession cancellationへ束縛し、Capability Brokerへ送る。
- Broker receiptを最小化したtool resultへ変換する。
...
- app-serverの`dynamicTools`は既存toolへ加算されるため、正のtool allowlistではない。`read-only`、prompt、`approvalPolicy=never`、`environments=[]`、clean profileも完全な隔離証拠として扱わない。
- production activationには、installed schemaでHost-only policyを確認し、ambient shell / MCP / app / plugin / extension / web / imageがdelegated turnへ1件も公開されないE2E readbackを必須にする。未知version、契約欠落、未承認versionではapp-server開始前にfail closedとする。
- Codex 0.145.0は`dynamicToolsOnly`または同等の正のallowlist契約を持たないため非対応とする。将来fieldが追加されても、公式仕様とWindows adversarial E2Eを別途承認するまでproduction gateを自動解除しない。
...
- AN3-B1はWindowsの実音声transportだけを扱う。installed Codex experimental schema、`account/read`、`thread/realtime/listVoices`をgateにし、明示click → Microphone → WebRTC offer / answer → remote audio / transcript → mute / stopを接続する。Voice root threadはread-only / approval never / tools禁止とし、Broker / MCPへ新しい経路を作らない。
- AN3-B2でCalendar read、Timer start、Today Focus、Calendar createを既存Capability Registry / Brokerへ接続し、承認と実行後readbackを加える。
- AN3-B2のproduction解除条件へ、Broker限定tool policy、Calendar separate grant、Timer approval single-flight / rate limit / cancellationを追加する。Codex 0.145.0では解除しない。
- AN3-B3でmacOS production adapterと両OS実機E2E、sleep / resume、device change、network / rate limit復旧を揃える。

完了条件:

- 「10分タイマーをかけて」で承認policyに従い1件だけ開始し、ID/readback一致。
- 「今日の予定を教えて」でtoday rangeだけ返す。
- Calendar grantがない場合はCalendar toolを公開せず、Provider呼出し0件で`permission_denied`になる。grantはSettingsのHost承認でだけ付与し、取り消し後はactive requestとtool定義へ即時反映する。
- Timer native approvalは同時1件、1分3件までとし、4件目、並行2件目、停止後のstale dialogを実行前に拒否する。
...
- model-visible tool surfaceがHoverPocket dynamic toolsだけであることを独立readbackし、ambient tool 0件を証明する。正のtool policy未対応または未承認versionではtransport factory呼出し0件のままblockedになる。
...
- AN5-A（PR #16をmainへmerge済み、merge commit `c8db98d`）: Host所有snapshot、validate / tests / preview、権限と実効Capability差分、exact approval、immutable version install、update、disable、preserve-only remove、rollback、active version / digest readback。最終reviewでrollback直前のsnapshot再保護も追加し、両OS CI成功後に取り込んだ。
...
- App health、unused Appのdisable提案、data retention。Capability監査ログ / receiptは両OSのSettingsから保持期間変更と確認付き削除を行い、実行済み墓標を残す。
...
- audit / receipt retentionをユーザーが`7日 / 30日 / 90日 / 無期限`から変更し、ネイティブ確認後に削除できる。削除後も同じplan / idempotency keyの副作用を再実行しない。
...
- offline / corrupted audit / malformed audit filename / symlink・reparse / receipt redaction後replay / sleep-wake / long-running soak。
...
| Voice標準provider | Codex app-serverを標準adapter、Realtime/BYOKを明示選択の代替adapterとする | Codexログインを再利用しAPIキーを不要にする。Capability契約は独立させ、正のBroker限定tool policyを確認できないversionではapp-serverを開始しない |
