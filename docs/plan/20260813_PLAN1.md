...
**ステータス**: 実装中・AN5-C runtime activationのPR #18は両OS CI成功。AN3-A Host-owned Voice Lane foundationをdefault-offで実装中
...
**現在地**: AN3-A - AN5-Cの最新headを土台に、全Provider共通のHost-owned Voice Lane、Compact / Expanded、root-scoped session card、app-server lifecycle foundationを両OSへdefault-offで実装中

**次のアクション**: AN3-AのWindows CIと最終security readbackを通してstacked PRを統合する。その後AN3-Bで、明示操作からのmicrophone / WebRTC、実Codex app-server schema probe、Calendar + TimerのBroker接続をWindows先行で実装する
...
- [ ] AN3: Voice LaneとCalendar + Timer縦断（AN3-A default-off UI / lifecycle foundationを実装中。実音声・Tool AdapterはAN3-B以降）
