...
- 2026-08-23の追加reviewを`7cb1764`、`b957a54`、`b34d576`、`1e6a8c8`、`3e8b79f`で修正した。macOSは3 ZIP、stable / versioned appcast、checksumの6資産をimmutable snapshotへ固定し、最終metadata再取得まで同一性を確認する。配布bundleの`SUFeedURL`、`SUPublicEDKey`、`TeamIdentifier=N7VVPW44ZA`もexact検証する。Windows betaはSetup SFX末尾とfull `.nupkg`全byte、Portable `current/`の506ファイルとfull package `lib/app/`を相対path / size / SHA-256で照合する。workflow path filterもmacOS native verifier変更時に起動する。
