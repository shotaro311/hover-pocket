...
updated: 2026-08-15
updated_by: codex
status: ai-native-in-progress; an2-merged; an3-main-synced; an3-isolated-macos-voice-e2e-ready; explicit-mic-turn-and-provider-e2e-pending; an4-pro-repair-pending
---

## 2026-08-15 AI-native AN3 Voice Foundation Integration

- 既存の実行中製品アプリとユーザーデータを変更せずにmacOS実音声gateを検証するため、DEBUG限定の隔離Voice E2Eアプリを追加した。毎回freshな0700 temp rootへ別bundle / dataを作り、bundle ID、実行名、Keychain namespaceを分離し、Google OAuth、Sparkle feed、installer launcherを無効化した。署名権限はmicrophoneだけに限定し、カメラ / Apple Events entitlementを持ち込まない。サニタイズ済みreceiptはavailability、session、root / transport、transcript件数、microphone / remote audioのbooleanだけを保存し、生transcript、SDP、token、path、ユーザーデータは保存しない。
- 隔離E2EアプリはCodex app-server ready、voice 19件、feature enabled、microphone未取得、remote audio未取得の安全な待機状態まで実機readbackした。普段使いのHoverPocket processは継続稼働し、隔離アプリ停止時も影響がなかった。Debug / Release warnings-as-errors、Broker、Capability、Voice / WebRTC、root-scoped sessions、Voice Lane layout、隔離data verifierがPASSし、Releaseでは同じE2E環境変数を渡してもoverrideが無効になることを実行確認した。固定working-tree snapshotのCodex Security scan `04214399-d7ba-4a97-8007-63ab89259da1`は11 / 11変更fileを完全レビューし、finding 0件、coverage completeでsealed completeとなった。実マイク / remote audio / 音声1往復 / safe closeはMacがlock中のため未実施であり、明示的なユーザー操作待ちである。
