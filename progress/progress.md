...
- 現在の公開ZIP成果物 `dist/releases/HoverPocket-0.1.0-73.zip` は Developer ID Application 署名と notarization/staple 済みで、GitHub Release `v0.1.0-73` に公開済み。latest appcast も build `73` を指す。一般ユーザー向けには同じ app-only payload を分かりやすい `HoverPocket-macOS-app.zip` として案内し、公開URLから再取得したZIPのトップレベルが `HoverPocket.app` のみであることを確認済み。SHA256 は `3f2c4adca44b284b961186172b4aa465263da8a79b22221d939894d96a21c44d`。
- Sparkle秘密鍵は macOS Keychain の `hover-pocket` アカウントにある。秘密鍵ファイルをGitに書き出さない。
- build `77` の配信は Apple Developer 契約承認待ち。`xcrun notarytool history --keychain-profile hover-pocket` が `HTTP status code: 403` と `A required agreement is missing or has expired` を返す間は、GitHub Release / Sparkle appcast を更新しない。現在の Latest は build `73`。
...
- 2026-06-29: Google OAuth を native/custom URL scheme と legacy/loopback の両方で OS 既定ブラウザ起動へ統一し、AppDelegate の `kAEGetURL` callback 受け渡しを追加。古い Chrome profile override 設定は README / `.env.example` / build script から削除。Calendar token/reconnect、外部モニター音量 DDC/CI VCP `0x62`、クラムシェル外部カメラなし Mirror 非表示も progress に記録。`swift build`、`git diff --check`、`./script/verify_google_calendar.sh` 成功。build `77` の配信は Apple Developer 契約未承認/期限切れによる notarytool 403 で停止し、未notarized ZIP は公開していない。
