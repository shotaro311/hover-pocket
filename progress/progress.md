...
- stacked Draft PR [#35](https://github.com/shotaro311/hover-pocket/pull/35)の初回Windows run [32672078767](https://github.com/shotaro311/hover-pocket/actions/runs/32672078767)はRelease buildからBrokerまで成功したが、Pocket Surface内の実helper childが10秒timeoutした。childは`Console.OpenStandardInput()`でredirect済みstdinを明示取得し、HostはCRLFではなくLFを明示書込みするよう修正した。`Environment.ProcessPath`が`dotnet` hostの場合のentry assembly再起動も追加し、修正後CIを次のreadback gateとする。
