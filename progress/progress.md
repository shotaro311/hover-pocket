...
updated: 2026-08-15
updated_by: codex
status: ai-native-in-progress; an2-merged; an3-cross-os-webrtc-verified; human-voice-roundtrip-and-provider-e2e-pending; an4-local-implementation-pending
---

## 2026-08-15 AI-native AN3 Voice Foundation Integration

- 最新head `91e2f46becd6db71d9d2452d0ab7cc2ec7e2b008`では、macOS / Windows双方の隔離Voice E2EがCodex app-server、Realtime v3、microphone、WebRTC transport、remote audio track / playbackまで実機で到達した。macOSは明示stop後にmedia / app-serverを解放し、Windowsも先行safe-stop検査で1.068秒以内にE2E shellとapp-server childが0件になり、既存製品0.2.7は継続した。Windowsの現在の隔離runは接続済みのまま人間発話待ちであり、user / assistant transcript各1件以上の1往復と、その後のsafe stopはまだ成功扱いにしていない。Macの人間発話1往復、両OSのVoice経由Calendar / Timer / Today Focus readbackも未完了である。
- Windows隔離E2Eの多重起動namespace、明示パネル表示、E2E限定taskbar可視化、安全終了待機を修正した。これらはDebugかつ明示E2E時だけ有効で、ReleaseはE2E flagを副作用前に拒否し、製品のno-activate / tool-window挙動は変更しない。最新headのPR [#6](https://github.com/shotaro311/hover-pocket/pull/6)はDraft、remote parity `0 / 0`、`MERGEABLE / CLEAN`で、Voice Windows [31869249790](https://github.com/shotaro311/hover-pocket/actions/runs/31869249790) / [31869252400](https://github.com/shotaro311/hover-pocket/actions/runs/31869252400)、Windows [31869252398](https://github.com/shotaro311/hover-pocket/actions/runs/31869252398)、macOS [31869252401](https://github.com/shotaro311/hover-pocket/actions/runs/31869252401)、PR Router [31869250442](https://github.com/shotaro311/hover-pocket/actions/runs/31869250442)が全成功した。
- Windows実機向けにexact base `2185b106126aaa526308afa48d70c1f34b75825a`から隔離branch `codex/an3-windows-voice-e2e-isolation`を作成し、clean review head `af6ecba3c84c23269463341010a2cf7aefe70c38`から安全修正を追加した。Debug限定のfresh temp root、製品版と別のmutex / open / stop event、中央ApplicationData resolver、Updater / installer / Google OAuth無効化、安全なE2E既定値、atomic sanitized receiptを維持し、ReleaseのE2E flagは製品root等へ到達する前に拒否する。Windows Settingsには既定オフ・Voice OFF時disabledのCalendar read opt-inを追加し、変更時はauthorization epoch失効、local transport cleanup、runtime OFF→ON再生成でtool権限を更新する。panel close中のin-flight native startもHost `_sessionGate`後にstopする。Debug 19 target + Release 1 targetの計20 Verifier、Debug / Release warnings-as-errors、JS 13件、PowerShell 5件、E2E Build、diff checkが成功した。この隔離基盤完成時点では実E2E GUI等を未実施としていたが、最新readbackは上記のとおり接続・microphone・remote audioまで進んでいる。詳細: `progress/2026-08/2026-08-15_hover-pocket-windows-an3-voice-e2e-isolation.md`。
- Mac Codex CLI 0.145.0実機でfeature未有効時のWebRTC startがRPC `-32600` / realtime非対応threadへ安全に分類された追加根拠を受け、macOS / Windows双方のHoverPocket production app-server既定だけへprocess-local `features.realtime_conversation=true` overrideを追加した。global Codex config、fake serverの明示引数、Releaseを含むVoice既定OFFは変更していない。Windows CLI 0.147.0ではoverride付きfeature list=true、schema / initialize / account / rate limits / 19 voiceのPhase 0がraw保持なしで成功し、production launch契約をCoordinator verifierへ固定した。追加差分後のWindows全20 verifier、両build、JS / PowerShell、E2E Build、diff checkは成功し、macOS buildはWindows環境のため未実施である。詳細は日別ログを正本とする。
- Mac Codex CLI 0.145.0実機の追加確定根拠では、feature有効でも`version=v1`はroot作成後SDP timeout、`version=v3` + `includeStartupContext=false`はWebRTC attach、remote audio playback、明示stop、媒体解放、app-server cleanupまで成功した。Windows production startをexact `v3` / `false`へ更新し、Fake app-server / Coordinator verifierへ独立したwire・literal検査を追加した。明示stopは同一root / client generationのexpected-stopとして非空closed reasonを正常終了にし、非期待closeは従来どおりエラーにする。変更後も全20 verifier、Debug / Release warnings-as-errors、JS 13件、PowerShell 5件、E2E Build、diff checkが成功し、GUI / マイクRunは未実施である。
- 既存製品データを変更せずmacOS実音声gateを検証するDEBUG限定の隔離Voice E2Eアプリを追加した。LaunchServices / TCCが同じ検証アプリを追跡できるようbundleは開発worktreeの安定した`dist/HoverPocketVoiceE2E.app`へ置き、保存データだけは毎回freshな0700 temp rootへ分離する。bundle ID、実行名、Keychain namespace、Google OAuth、Sparkle、installer launcherも製品版から分離し、署名権限はmicrophoneだけに限定した。receiptはavailability、session、root / transport、transcript件数、microphone / remote audioのbooleanだけを保存し、生transcript、SDP、token、path、Provider dataは保存しない。
- Compact / Expandedの実画面は`600x494`から`600x650`へ下方向だけに伸び、幅と既存Provider領域が不変であることをComputer Useで確認した。明示mic clickからmacOS native `AVCaptureDevice`許可要求まで到達し、許可後だけsingle-use WebView grantとWebRTCを開始する順序へ修正した。この検証時点の隔離receiptはsession=`requestingPermission`だったが、最新readbackではmicrophone、remote audio、safe closeまで成功している。
- head `ad7fe0fc7d19c8ae5ba438314f5951bb32841964`ではfake app-serverの起動待ちを5秒へ安定化し、ローカル20連続PASS後、PR Router [31859441859](https://github.com/shotaro311/hover-pocket/actions/runs/31859441859)、Windows [31859443683](https://github.com/shotaro311/hover-pocket/actions/runs/31859443683)、macOS [31859443675](https://github.com/shotaro311/hover-pocket/actions/runs/31859443675)、Voice Windows [31859443681](https://github.com/shotaro311/hover-pocket/actions/runs/31859443681) / [31859441104](https://github.com/shotaro311/hover-pocket/actions/runs/31859441104)が全成功した。native microphone gateを追加したhead `eaf8db49467806f0cfcb4a604aa53389fc8bc295`でもPR Router [31860149039](https://github.com/shotaro311/hover-pocket/actions/runs/31860149039)、Windows [31860150992](https://github.com/shotaro311/hover-pocket/actions/runs/31860150992)、macOS [31860150901](https://github.com/shotaro311/hover-pocket/actions/runs/31860150901)、Voice Windows [31860148084](https://github.com/shotaro311/hover-pocket/actions/runs/31860148084) / [31860150830](https://github.com/shotaro311/hover-pocket/actions/runs/31860150830)が全成功した。
- 隔離E2E実装head `d916c1dbfa8f69b25d11b82948626bd4b68e52eb`をDraft PR #6へpushした。remote parity 0 / 0、worktree cleanをreadbackし、Windows Voice CI [31851023897](https://github.com/shotaro311/hover-pocket/actions/runs/31851023897) / [31851024054](https://github.com/shotaro311/hover-pocket/actions/runs/31851024054)、Windows通常verify [31851024094](https://github.com/shotaro311/hover-pocket/actions/runs/31851024094)、macOS verify [31851024055](https://github.com/shotaro311/hover-pocket/actions/runs/31851024055)、PR Router [31851022775](https://github.com/shotaro311/hover-pocket/actions/runs/31851022775)はすべて成功した。PR #6はDraft、MERGEABLE / CLEANである。
- AN2 Ready PR [#9](https://github.com/shotaro311/hover-pocket/pull/9)を全check成功、MERGEABLE / CLEANのreadback後にmergeした。`origin/main`はmerge commit `014032d412ab488c5e526f1ed2e7d23218c38a87`となり、AN3 branchへ通常merge commit `90bd31fd9a772387027add6c93414d7882b3eed5`で同期した。Draft Voice PR #6は引き続き未マージである。
- current rootと同じsession treeだけを表示するchild / descendant session cardsをmacOS / Windowsへ接続した。ChatGPT Pro Critic run `20260815-064708-hoverpocket-an3-root-scoped-session-cards`の本文内`critic-review.md`が指摘したP1 2件 / P2 3件 / P3 2件を実コードで再現し、上限付きpagination、read失敗からの回復、conflicting duplicate全拒否、root / child UI IDの名前空間化、Windows WebRTC失敗後のroot / polling / tool authority破棄、timestamp parity、決定的fixture強化をhead `0ca7834b07795a8b546ea125ac8928b730f67800`へ反映した。Proの本文hashは`d4f5b984...618ae`で一致したが、artifact parserは別ファイル抽出に失敗したため、receipt上のartifact状態は`blocked`として保持する。
- macOS実機用の内容非表示live probeはCodex CLI `0.145.0`、initialize、ログイン済みaccount、rate limit API、19 voice、default voice、malformed / unknown protocol 0件でPASSした。Windows実機の隔離read-only gateもCodex CLI `0.147.0`、schema、phase0、rate-limitsの`params`省略、19 voice、Debug / Release 0 warning / 0 error、18 / 18 Verifier、process残存0、remote parity / cleanでPASSした。head `0ca7834`のWindows Voice CI 2本、通常Windows、macOS、PR Routerも全成功した。残るAN3 gateは両OSの実マイク / remote audio / 音声1往復 / safe close、Voice経由の実Calendar / Timer / Today Focus readbackである。
...
- 実装head `a82291618ee7c8e9939b4f611f694c13ac9a9fa3`ではVoice Lane Windows CI [31841209115](https://github.com/shotaro311/hover-pocket/actions/runs/31841209115) / [31841213311](https://github.com/shotaro311/hover-pocket/actions/runs/31841213311)、通常Windows [31841213324](https://github.com/shotaro311/hover-pocket/actions/runs/31841213324)、macOS [31841213322](https://github.com/shotaro311/hover-pocket/actions/runs/31841213322)、PR Router [31841211479](https://github.com/shotaro311/hover-pocket/actions/runs/31841211479)が全成功した。その後root-scoped cardsとmacOS live probeを追加しており、現在残るAN3 gateは両OS実音声E2E、実Calendar / Timer / Today Focus readback、Windows installed schema probeである。
...
- Windowsのapplication-lifetime Voice runtimeへaccount / voice capability gate、詳細状態機械、bounded restart / backoff、single-use microphone permission、WebRTC SDP交換、remote audio、mute / stop / safe closeを接続した。初期実装head `91aa8d3`では当時のschema根拠により`v1`だったが、Mac実音声PASSを正本として現在は`v3` / startup contextなしへ更新した。root threadはread-only sandbox / approval never / isolated workspaceで開始する。対象Windows実機のマイク・実音声1往復と、VoiceからBrokerへのtool dispatchは次のgateである。
