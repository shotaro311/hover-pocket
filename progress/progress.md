...
- installed schemaを起動前に検査し、`dynamicToolsOnly`の正のBroker限定policy、解決済み実行ファイルURL、size / mtime / inode由来identityを固定する。同一identityをspawn直前とbounded restartごとに再確認し、version probeは5秒、schema生成は15秒で停止する。
...
- 現行Codex `0.145.0`は`dynamicToolsOnly`を持たないため、foundation verifierは成功するがinstalled readinessは終了コード2、`codex_broker_only_tool_policy_missing`でBLOCKEDとなる。app-server process、microphone、Calendar / Timerは開始しない。対応Codexでの物理マイク、SDP、remote audio、tool callの実E2Eは未完了であり、推奨 / 動作済み / 公開可能とは扱わない。
