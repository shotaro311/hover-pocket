...
- Homebrew Codex `0.145.0`は追加tool混入でroute gateを停止し、隔離した公式`0.149.0`はroute / account / 19 voicesまで成功後、現行backendとの`session.model`契約差でSDP開始を即時停止する。一方、ChatGPT.app同梱Codex `0.150.0-alpha.12.2`は環境変数なしでaccount、19 voices、ephemeral thread、SDP、WebRTC、teardownを全通過した。最終修正後に独立processで3回反復し2.171〜2.312秒、失敗0、一時workspace残存0だった。物理マイク、remote audio、transcript、Calendar / Timer tool callの実E2Eは引き続き未完了であり、公開可能とは扱わない。
