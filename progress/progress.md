...
- Debug / Release warnings-as-errors、Voice Foundation runtime、静的42 contract、Panel 128 cases、Capability 20 handlers、`git diff --check`、通常bundle build / launch / strict codesignがPASSした。独立レビューはP0 / P1 / P2すべて0件で、追加I/O、polling、probe、新規session起動はなく、過剰な安全実装やVoice hot path性能低下を認めなかった。
- Voice sessionへ公開中のCapability toolは`calendar_events_list`、`calendar_event_create`、`timer_countdown_start`の3つだけである。Calendar作成とTimer開始はネイティブ承認後にBroker readbackを要求し、RegistryにあるSticky Notes / Clipboard / Controls / Calculator / Timer pause・stop等はまだVoice toolとして公開していない。詳細: `progress/2026-09/2026-09-01_hover-pocket-normal-ui-voice.md`。
