...
- 独立エージェントの安全性 / 性能レビューは最終再レビューでP0 / P1とも0件。Broker限定policy、read-only sandbox、空workspace roots、非永続WebView、CSP、5秒microphone leaseは必要な境界として維持した。一方、ambient要求だけを入力順に短く隔離し、通常tool本体は並行実行するため、stdout停止や応答遅延を増やさない。pending / current / 旧clientはidentityとgenerationで分離し、隔離済みclientは再起動しない。
- app-server専用profileの追加差分も別エージェントが再レビューし、API key認証の誤受入れと一過性probe失敗の無期限cacheを検出・修正した。最終P0 / P1は0件。公式0.149のroute canaryは0.27〜0.65秒でVoice設定時に1回だけ、mic開始hot pathには入らない。残るP2はkeyring-only Codex loginの継承未対応で、現環境のowner-only `~/.codex/auth.json`はsymlink先と`account.type=chatgpt`をreadback済みである。
...
- 現行Codex `0.145.0`は実canaryで追加の`update_plan`をoutbound requestへ含めるため、foundation verifierは成功するがinstalled readinessは終了コード2、`codex_broker_only_tool_route_mismatch`でBLOCKEDとなる。隔離した公式Codex `0.149.0` exact binaryは同じcanaryを約0.65秒で通過し、ChatGPT accountと19 voiceもreadbackしたが、system installは変更していない。対応versionでの物理マイク、SDP、remote audio、Calendar / Timer tool callの実E2Eは未完了であり、動作済み / 公開可能とは扱わない。
- code head `0a09b72a7c24baf0034cf26e15bfce5e1e0c276b`のDraft PR #39は、Router [33282321608](https://github.com/shotaro311/hover-pocket/actions/runs/33282321608)、macOS [33282323019](https://github.com/shotaro311/hover-pocket/actions/runs/33282323019)、Windows [33282323022](https://github.com/shotaro311/hover-pocket/actions/runs/33282323022)、3OS contract / compare [33282323021](https://github.com/shotaro311/hover-pocket/actions/runs/33282323021)を含む15 check成功・8 gate skip・失敗0・pending 0で、`MERGEABLE`である。merge / releaseは実施していない。
...
- GitHub releaseと`macos-latest`は更新していない。一般公開版は引き続き`0.1.0 (168)`である。build 583はDraft PR #39の未公開候補であり、物理マイク / OpenAI Realtime / remote audioのユーザー確認、実モデル生成、PR Ready / merge判断が完了するまで公開しない。
