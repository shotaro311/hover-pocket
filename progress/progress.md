...
- このMacには.NET SDK / PowerShellがないため、final headのWindows Release / Debug build、native credential broker、Voice E2E isolation、PowerShell構文、rendered WebView2は新しいDraft PR CIを必須gateにする。両OSの実マイク・可聴remote audio、macOS notarization / Gatekeeper / Sparkle、Windows timestamped Authenticode / Velopack / feed、実配布rollback、stack mergeは未完了である。詳細: `progress/2026-08/2026-08-26_hover-pocket-ai-native-final-integration.md`。
