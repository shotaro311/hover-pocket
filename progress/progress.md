...
- 同headのmacOS Capabilities [33228860545](https://github.com/shotaro311/hover-pocket/actions/runs/33228860545)は、既存の10ms timeout対100ms handler fixtureだけが`timeout_status`で失敗した。直前19 runとローカル30回は成功しており、macOS / Windows双方のfixtureをtimeoutでcancelされるまで30秒待つ形へ変更した。Swift buildと修正後50回連続のBroker verifierは成功した。更新headの全CIを次の受入gateとする。
