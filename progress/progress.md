...
- macOS warnings-as-errors build、Panel layout 128件、Capability 20 handler、Broker 21 descriptor / 20 handler、Pocket App / Surface / Voice / Timer、Pocket contract `14 schema / 69 fixture`、Voice contract、Windows JavaScript構文、`git diff --check`が成功した。このMacに.NET SDKはないため、Windows Release buildとnegative verifierはPR CIを最終gateにする。
