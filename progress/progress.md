...
- Compact / Expandedの実画面は`600x494`から`600x650`へ下方向だけに伸び、幅と既存Provider領域が不変であることをComputer Useで確認した。明示mic clickからmacOS native `AVCaptureDevice`許可要求まで到達し、許可後だけsingle-use WebView grantとWebRTCを開始する順序へ修正した。現在の隔離receiptはCodex app-server ready、voice 19件、session=`requestingPermission`、microphone / remote audio未取得で、TCCの明示許可待ちである。Debug / Release warnings-as-errorsとapp-server verifierはPASSし、実マイク / remote audio / 音声1往復 / safe closeは未完了である。
