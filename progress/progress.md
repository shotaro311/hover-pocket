...
- macOSでSwift warnings-as-errors build、Pocket App lifecycle、Pocket Surface、Capability、Broker、Panel layout、Timer、Calculator、Clipboard、Weather、共通contract 12 schema / 58 fixtureの2回byte一致、`git diff --check`が成功した。公開Capability schemaも末尾改行を真の終端で拒否し、runtimeとの不一致をnegative fixtureへ固定した。PR [#16](https://github.com/shotaro311/hover-pocket/pull/16)のsource head `c949676`でWindows [31897102514](https://github.com/shotaro311/hover-pocket/actions/runs/31897102514)、macOS [31897102528](https://github.com/shotaro311/hover-pocket/actions/runs/31897102528)、3OS contract / cross-OS compare [31897102509](https://github.com/shotaro311/hover-pocket/actions/runs/31897102509)、PR Router [31897110408](https://github.com/shotaro311/hover-pocket/actions/runs/31897110408)が成功した。未解決review threadは0、`MERGEABLE / CLEAN`、remote parity `0 / 0`をreadbackした。PRは自動mergeしていない。
- 承認previewの実byte再検証、grantのrequest ID binding、remove recordのfile / directory durable sync、manager破棄時のstaging ownership解放、両OSのstable key true-end anchorとASCII制御文字拒否まで追加hardeningした。exact range `16e7cda...88f41bd`のscan `f62b2099-b34d-4ce9-8609-5f514aa90358`は5 / 5 file、runtime最終range `88f41bd...3003bb9`のscan `38a3798b-2171-4980-9db7-59492e69c7ff`は1 / 1 fileを閉じ、どちらもcoverage complete、reportable finding 0件でsealed completeとなった。その後、公開descriptorの末尾改行許容をreviewで検出し、`c949676`でschemaとruntimeを一致させた。保存先rootの既知pathname TOCTOUはproduction接続前の別gateとして残す。
