...
- Windows実機向けにexact base `2185b106126aaa526308afa48d70c1f34b75825a`から隔離branch `codex/an3-windows-voice-e2e-isolation`を作成し、Debug限定のfresh temp root、製品版と別のmutex / open / stop event、中央ApplicationData resolver、Updater / installer / Google OAuth無効化、安全なE2E既定値、atomic sanitized receiptを実装した。WebView2は明示Voice click後だけ既存single-use microphone gateを使い、mic / remote track / playback成功・失敗 / transport / safe closeを型付き通知する。Debug / Release warnings-as-errors、Windows全19 Verifier、Release override無効、JS 13件、PowerShell 5件、E2E Build、diff checkが成功した。実E2E GUI、Windows privacy許可、マイク、remote audio、音声1往復、Run / Readback / Stopは次の明示実機gateとして未実施で、インストール済み0.2.7と実行中製品processは変更・停止していない。詳細: `progress/2026-08/2026-08-15_hover-pocket-windows-an3-voice-e2e-isolation.md`。
