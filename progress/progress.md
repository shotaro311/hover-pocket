...
- Macには`dotnet`と`pwsh`がないため、C# Release / Debug build、Settings verifier、PowerShell parser / self-test / nonexistent-drive fail-closedはPRのWindows CIで検証する。CI readback前はfindingをfixed扱いしない。
