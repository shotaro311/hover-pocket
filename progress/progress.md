...
- contract corpusは12 schema / 47 fixtureへ増え、全reject fixtureが本文digest、stable error code、exact error locationを持つ。監査ログも既知Invocation、descriptor、入力digest、Host-owned readback digestへbindingする。contract treeはLFへ固定する。CIは固定runner / Python / Action SHAを使い、Ubuntu / macOS / Windowsのreport artifactをbyte-for-byte比較する。
