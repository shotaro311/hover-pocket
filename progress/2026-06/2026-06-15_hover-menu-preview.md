...
- Calendar 表示初期のラグ対策として、保存済みGoogle認証の確認中でもCalendar本体を先に表示する `restoring` 状態を追加。予定データ取得前でも空の日付グリッドを即描画し、Google Calendar取得は背後で更新するようにした。
- Calendar 日付セル生成を軽量化。予定データ未取得時の空グリッドを月単位でキャッシュし、日付ID生成で毎セル `DateFormatter` を作らないようにした。
...
- Calendar 初期表示レスポンス改善後に `swift build`、`git diff --check`、`./script/build_and_run.sh --verify` 成功。
