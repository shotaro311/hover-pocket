...
- Pro runはgeneration 2のlocal verificationと7 acceptanceを全件PASSにして`complete`へfinalizeし、terminal receiptとdeliveryの`synthesis_completed_at`を再読込した。同じdeliveryは再適用しない状態である。
- 既存公開release readback [33256489278](https://github.com/shotaro311/hover-pocket/actions/runs/33256489278)はWindows package identityとmacOS署名 / notarization / Gatekeeperを成功した。macOS公開版のinstall / rollback transitionは [33256490541](https://github.com/shotaro311/hover-pocket/actions/runs/33256490541) で成功した。
- 同runのWindows実transitionは、GUI executableが`$LASTEXITCODE`を設定しない条件を初めて実行して失敗した。`ProcessStartInfo`からstdout / stderr / `ExitCode`を決定論的に回収し、成功出力とexit 7の拒否をPowerShell契約へ追加した。同じ問題を持つformal publishのrelease-config検証も同時に修正した。
- 2回目は公開済みアプリへbuild-timeのGoogle OAuth照合環境値を要求していたためexit 1となった。build-time OAuth照合はpublish工程に維持し、公開transitionは独立取得したProductVersionとアプリ内Updater verifierを使うよう責務を分離した。
- 最終head `f9fa4267f237c7e6e1bd7780c46e68d5a2a277a2`はWindows [33256921874](https://github.com/shotaro311/hover-pocket/actions/runs/33256921874)、macOS、3 OS contract、release / transition contractを含む13 check成功・11 gate skip・失敗0、Draft / CLEANである。
- 修正後の実transition [33256955463](https://github.com/shotaro311/hover-pocket/actions/runs/33256955463)は公開Windows beta `win-v0.2.6`のinstall、`win-v0.2.7`へのupgrade、旧版rollback、再upgrade、uninstall、reinstall、user data保持をartifact receiptで全件`verified`としてreadbackした。
- 正式publisherで署名したMSI/helperの生成・公開とそのinstall / upgrade / rollback / uninstall、通常Windows hostのUAC、formal Authenticode readback、物理Voice / 実モデル生成は未完了であり、Draft PR #39をReady / mergeにせずproduction機能をOFFで維持する。
