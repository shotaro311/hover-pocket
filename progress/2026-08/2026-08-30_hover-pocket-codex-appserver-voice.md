...
## Provider-bound Codex物理Voice gate

- ChatGPT Pro Criticの成果物をdelivery / state hash / artifact hashで回収・受入し、AN8初回実用リリースは現時点でNO-GOと判定した。P0は0件で、優先修正は物理Voice証拠のProvider拘束とexact-head証跡だった。同一deliveryはmark-done済みで、重複適用しない。
- macOS / Windowsの隔離E2Eを`codex_app_server`へ固定した。Provider変更時はmedia、transcript、Timer readback、Host所有の物理確認、attempt / leaseを失効し、Codexへ戻しただけでは復活しない。新しいCodex media attempt後だけ証拠を再取得できる。
- 決定論的fixtureへ`Codex確認済み -> BYOK / Off -> Codex`の往復負例、旧attempt拒否、fresh attempt回復を追加した。macOS単体receipt verifierはphysical stageでCodex以外を無条件拒否し、BYOKをexpected providerにした負例も持つ。
- Debug / Releaseのwarnings-as-errors build、Voice E2E isolation、Voice Foundation、Capability 20、Broker 21 descriptor / 20 handler、Timer、receipt self-test、Voice静的42件、shell構文、`git diff --check`がPASSした。このMacに.NET SDK / PowerShellはないが、Draft PR #39のWindows CIでC# Release / Debug build、Voice E2E isolation、PowerShell構文、rendered WebView2を含む全stepが成功した。
- current HEADからfresh provider-bound sessionをBuild / Runし、process / storage所有、明示Provider拘束、Voice既定OFF、receipt / performance、safe close、Stop、CleanupをHarnessでreadbackした。検証用PID `98523`だけを安全停止してTrashへ移し、長時間基準PID `70741`と一意表示名の旧build 608 PID `86913`は生存を確認した。
- 旧build 608はprovider-bound schemaより前のため`legacy_nonphysical_only`とし、物理Voice合格証拠には使わない。次の人手gateではcurrent exact HEADから新しい一意表示candidateを作り、物理マイク、可聴remote audio、user / assistant transcript、Timer承認 / readbackを確認する。
- 独立エージェントの修正後レビューはP0 / P1 / P2すべて0件。新処理は隔離E2E storeだけに限定され、通常起動、Hover、通常VoiceのCPU、I/O、レイテンシへの影響はない。merge、Draft解除、release、公開は実施していない。
- 実装commit `d50bdadb73d10ea4860c2366e76985ff0577280d`をDraft PR #39へpushし、CIは15 SUCCESS / 8 expected SKIPPED / failure 0 / pending 0だった。PRはDraft / OPEN / MERGEABLEで、remote parityは0 / 0。merge、Draft解除、release、公開は行っていない。

## exact runtime HEAD build 615と証拠manifest

- runtime source HEAD `9a110e9ae260ec65f7c99496baf36ff8c899b250`はremote parity `0 / 0`で、Draft PR #39はDraft / OPEN / MERGEABLE / CLEAN、review / comment 0件だった。同一SHAのWindows run `33307549713`、macOS run `33307549728`、3 OS contract / compare run `33307549725`は成功し、PR全体は11 SUCCESS / 8 expected SKIPPED / failure 0 / pending 0だった。Windows runのworkflow artifact uploadは0件で、CI buildと正式配布artifactを混同しない。
- 同HEADを`0.1.0 (615)`としてDeveloper ID署名・Apple公証した。submission `2f2ead18-b466-4831-844d-3fcffb42ff54`は`Accepted`。ZIP `HoverPocket-0.1.0-615.zip`は`7511492` bytes、SHA-256 `2901a0136531dff1834dee26c0cf14649e059b4f323e5c3293d37e516b3266d9`である。bundle ID `local.codex.hover-pocket`、version `0.1.0`、build `615`、strict codesign、stapler、Gatekeeper `Notarized Developer ID`、release entitlements 3件を再readbackした。
- 署名後main SHA-256は`1a16e6b7b049fff89f433087074840014547073e31a586c2b11f9847cba66057`、Release source binaryは`25fbfd5ec6342042c5b7d2108da92f992ce20e5201372b3541832fa38e1113da`である。Mach-O UUIDは`45B20385-18C5-30FD-ACCD-04D272923CA3`。Sparkle `2.9.3`と`libMediaRemoteAdapter`のdependency closure、appcast build / length / versioned URL / EdDSAを確認し、公開は行っていない。
- build 615配布binaryでVoice OFFの100回open / close、Timer / Calculator 100 switch、5 recovery、3 animated transitionを実行した。window `3->3`、thread `15->10,max=17`、RSS `115.781->128.250 MiB`、socket `0->1`、child `0->0`で、64 MiB上限内だった。Capability 20、Broker 21 descriptor / 20 handler、Pocket Surface、Pocket App package / lifecycle / generation / migration / health / workspace backup、Voice Foundation、Voice E2E isolationも成功した。
- 同じ配布binaryからCodex app-server ChatGPT account / 19 voices / ephemeral thread / SDP / WebRTC / process teardownを通し、`gpt-5.6-sol` / `medium`のTimer tool、Broker承認1回、readback、process teardownも確認した。OpenAI API key、実マイク、remote audio、transcript、Calendar createは使用していない。
...
## 隔離Voice候補のChatGPTログイン修正とbuild 618

- build 617の設定画面ではCodex app-server選択中でも「共有しているCodexログインではChatGPTアカウントを確認できません」と「再確認」だけが表示され、「ChatGPTでログイン」が存在しなかった。実runtimeのVoice専用Codex Homeにも`auth.json`はなく、Hostのowner-only `~/.codex/auth.json`は存在したが、隔離runtimeは`externalIntegrationsEnabled=false`のため認証storageを`.disabled`へ固定していた。
- これは、macOS Voice E2Eの実機gateでユーザーがCodex app-serverへChatGPTログインする要件との不一致だった。認証policyを`disabled / managedOnly / externalOrManaged`へ分け、隔離Voice E2Eだけ`managedOnly`を選ぶ。本番の外部credential共有優先は変えず、隔離候補はHostの`~/.codex/auth.json`、Keychain、通常版HoverPocketの認証を参照・symlink・変更しない。
- `CodexAppServerVerificationCommand`へ、外部source fileが存在しても`managedOnly`ではsymlinkを作らずfresh managed profileを返す負例を追加した。`MacOSVoiceE2EIsolationVerificationCommand`はprofileがruntime root配下、managed login有効、継承credentialなし、external linkなしを確認する。
- `voice_e2e_macos.sh ValidateIsolation`は`CodexVoiceAppServer`をtop-level allowlistへ追加し、profile root / Codex Home / configのownerと`0700 / 0600`、存在するmanaged credentialのregular file、owner、`0600`、hardlink数1を確認する。physical stageは専用credentialがなければ固定エラーで停止する。
- `swift build -Xswiftc -warnings-as-errors`、Release warnings-as-errors、`--verify-codex-app-server`の管理ログイン4シナリオ・6 process、`--verify-voice-e2e-isolation`、Voice Foundation、Panel 128 cases、Voice静的42 cases、shell構文、`git diff --check`がPASSした。
- fresh ad-hoc候補`ホバーポケット Voice E2E 618`はsession `HoverPocketVoiceE2ESession-1lQSaT`、runtime `HoverPocketVoiceE2E-dSiGNi`、PID `25480`。strict codesign、Run / Readback / ValidateIsolation、Voice既定OFF、provider未要求、mic / remote audio / Timer / physical confirmationなしを確認した。SettingsのAccessibility treeには「HoverPocket専用のCodexプロファイルは未ログインです。」「ChatGPTでログイン」「再確認」が表示された。profile / Codex Home / configはcurrent-user所有の`0700 / 0700 / 0600`、専用`auth.json`はログイン前なので未作成で、Host authのmetadataは変更していない。実ブラウザログインはまだ開始していない。
- 旧build 617 PID `19758`はHarness Stopで`safe_close`、process停止、receipt / performance保持を確認し、一時session / runtime / buildをTrashへ移した。物理Voice gateは一意表示名build 618だけへ引き継ぐ。
- 独立レビューはP0 / P1 0件で、コード上の残存P2も0件。Host auth非参照、runtime内`CODEX_HOME / HOME`、regular / non-symlink / owner / private permission / hardlink readback、Cleanup、本番`externalOrManaged`不変、通常起動 / Voice hot pathへの継続負荷なしを確認した。Harness usageだけが旧「logged-in Codex account共有」説明だったP2は、隔離profile内の個別ChatGPTログインとHost credential非共有へ更新した。
- 実装commit `c96457c7778a3767da8639d8bf6d0bebf20df3ea`のDraft PR #39は、macOS / Windows / 3 OS契約 / routerを含む15 SUCCESS / 8 expected SKIPPED / failure 0 / pending 0。Draft / OPEN / MERGEABLE、remote parity 0 / 0で、merge、Draft解除、公開、releaseは実施していない。
