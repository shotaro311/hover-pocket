    ...
    : ICapabilityApprovalPresentationResolver
{
        ...
        CapabilityApprovalRequest request)
    {
        ...
        foreach (var pair in plan.Steps.Zip(descriptors))
        {
            ...
            var displayLabel = note is null
                ? null
                : SanitizedDisplayLabel(note.Title) ?? SanitizedDisplayLabel(note.Body);
            if (note is not null && displayLabel is null)
            {
                throw new CapabilityBrokerException("CAPABILITY_PLAN_INVALID", "approval_target_label");
            }
