...
final class ControlsCapabilityHandler: PocketCapabilityHandler {
    ...
    private func mediaOutput(command: String, state: ControlsNowPlayingState) -> CapabilityObject {
        [
            ...
            "safeTitle": .string(state.title.sanitizedCapabilityVisibleText(maximumCount: 160)),
            "safeSource": .string(state.sourceName.sanitizedCapabilityVisibleText(maximumCount: 120))
        ]
    }
