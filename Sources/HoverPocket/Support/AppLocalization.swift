...
enum AppText {
    ...
    private static func japaneseText(_ key: AppTextKey) -> String {
        ...
        case .weatherRegionDetail: return "現在地、世界の都市・郵便番号、または日本の都道府県から選べます。選択地点と直近の予報はこのMacに保存され、取得できない時は保存済み予報を表示します。位置情報は「現在地を使用」を押した時だけ取得します。"
    ...
    private static func englishText(_ key: AppTextKey) -> String {
        ...
        case .weatherRegionDetail: return "Choose the current location, search worldwide by city or postal code, or select a Japanese prefecture. The location and latest forecast are saved on this Mac for offline display. Location access is requested only when you choose to use it."
