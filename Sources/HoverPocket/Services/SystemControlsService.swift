...
private final class SystemVolumeService: @unchecked Sendable {
    ...
    func currentState() async -> ControlsVolumeState {
        await Task.detached(priority: .utility) { [self] in
            readCurrentState(allowRememberedMonitorFallback: true)
        }.value ?? .empty
    }

    func capabilityState() async -> ControlsVolumeState? {
        await Task.detached(priority: .utility) { [self] in
            readCurrentState(allowRememberedMonitorFallback: false)
        }.value
    }
    ...
    func setVolume(_ level: Double, unmute: Bool = true) async {
        ...
        await Task.detached(priority: .utility) { [self] in
            let wasMuted = readCurrentState(allowRememberedMonitorFallback: true)?.isMuted ?? false
    ...
    private func readCurrentState(allowRememberedMonitorFallback: Bool) -> ControlsVolumeState? {
        let deviceID = Self.defaultOutputDevice()
        ...
        if shouldReadMonitorVolume || coreAudioVolume == nil || !canSetCoreAudioVolume {
            ...
            if allowRememberedMonitorFallback,
               shouldReadMonitorVolume,
               ddcBridge.hasAddressableExternalDisplay() {
